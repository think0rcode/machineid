...
func getSMBIOSUUID(ctx context.Context) (string, error) {
	...
	slog.Debug("powershell CIM output", "output_len", len(out))

	if u := firstUUID(out); u != "" {
		slog.Debug("SMBIOS UUID found via PowerShell CIM", "uuid_redacted", redactID(u))
		return u, nil
	}
	...
	slog.Debug("powershell WMI output", "output_len", len(out))

	if u := firstUUID(out); u != "" {
		slog.Debug("SMBIOS UUID found via PowerShell WMI", "uuid_redacted", redactID(u))
		return u, nil
	}
	...
	slog.Debug("wmic output", "output_len", len(out))
	if u := firstUUID(out); u != "" {
		slog.Debug("SMBIOS UUID found via WMIC", "uuid_redacted", redactID(u))
		return u, nil
	}
...
func getInstallationID(_ context.Context) (string, error) {
	...
	if guid == "" {
		slog.Error("MachineGuid is empty", "empty", true)
		return "", errors.New("machine guid empty")
	}

	slog.Debug("installation ID found via registry", "guid_redacted", redactID(guid))
